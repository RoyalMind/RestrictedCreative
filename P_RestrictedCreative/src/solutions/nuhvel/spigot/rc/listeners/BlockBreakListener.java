...
public class BlockBreakListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (new PreconditionChecker(plugin, player).isAllowedWorld(b.getWorld().getName()).anyFailed())
            return;
        ...
        if (player.getGameMode() == GameMode.CREATIVE &&
                !new PreconditionChecker(plugin, player).isDisabledBreaking(m).anyFailed()) {
            getMain().getUtils().sendMessage(player, true, "disabled.general");
            e.setCancelled(true);
            return;
        }

        if (new PreconditionChecker(plugin, player).isExcludedFromTracking(b.getType()).anyFailed())
            return;
